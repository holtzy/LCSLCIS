...
bool VERBOSE=false; // turn it true to make th eprogram more verbose
...
void linearExtentionCode(std::vector<std::vector<std::string> > oh1, std::vector<std::vector<std::string> > oh2, std::vector<std::string> & ext1, std::vector<int> & posExt2){
	...
	int ind = 1;
	for (int i = 0; i < (int)oh2.size(); i++){
		for (int j = 0; j < (int)oh2[i].size(); j++){
	...
	//int posExt1 = 0;
	for (int i = 0; i < (int)oh1.size(); i++){
		for (int j = 0; j < (int)oh1[i].size(); j++){
...
std::vector<std::vector<std::string> > Homogeneization(std::vector<std::vector<std::string> > o1, std::vector<std::vector<std::string> > o2){
	...
	for (int i = 0; i < (int)o2.size(); i++){
	 ...
	 // Sort Ltemp according to second positions of pairs
			...
			for (int k = 0; k < (int)Ltemp.size(); k++){
...
List LCS(DataFrame order_1, DataFrame order_2){
	...
	//Positions of items starting at 1 stored in these dictionaries
	...
	for (int j = 1; j < order_1.nrows(); j++){
		...
		//Make sure that o1 is sorted
		if ((j>1) && posO1[j] < posO1[j-1]){
			std::cout << "LCS ALERT : o1 not sorted"<<std::endl;
			return 0;
		}
		last = posO1[j];
	}
	...
	std::cout << "o1 gets " << numBuc1+1 << " buckets and " << order_1.nrows() << " markers." << std::endl;
	...
	for (int j = 1; j < order_2.nrows(); j++){
		...
		//Make sure that o2 is sorted
		if ((j>1) && posO2[j] < posO2[j-1]){
			std::cout << "LCS ALERT : o2 not sorted"<<std::endl;
			return 0;
		}
		last = posO2[j];
	}
	...
	std::cout << "o2 gets " << numBuc2+1 << " buckets and " << order_2.nrows() << " markers." << std::endl;
	...
	for (int i = 0; i < (int)oh2.size(); i++){
		...
		if(VERBOSE){
			for (int j = 0; j < (int)oh2[i].size(); j++){
				std::cout << oh2[i][j];
				if (j != (int)oh2[i].size()-1) std::cout << " ";
			}
			std::cout << ")";
		}
	}
	...
	for (int i = 0; i < (int)oh1.size(); i++){
		...
		if(VERBOSE){
			for (int j = 0; j < (int)oh1[i].size(); j++){
				std::cout << oh1[i][j];
				if (j != (int)oh1[i].size()-1) std::cout << " ";
			}
			std::cout << ")";
		}
	}
...
void bucketCode(std::vector<std::vector<std::string> > oh1, std::vector<std::vector<std::string> > oh2, std::vector<int> & posBuc2){
	...
	if(VERBOSE){
		for (int i = 0; i < (int)posBuc2.size(); i++){
			std::cout << "posBuc2_trace[" << i << "] = " << posBuc2[i] << std::endl;
		}
...
List LCIS(DataFrame order_1, DataFrame order_2){
	...
	for (int j = 1; j < order_1.nrows(); j++){
		...
		//Make sure that o1 is sorted
		if ((j>1) && posO1[j] < posO1[j-1]){
			std::cout << "LCIS ALERT : o1 not sorted"<<std::endl;
			return 0;
		}
		last = posO1[j];
	}
	...
	for (int j = 1; j < order_2.nrows(); j++){
		...
		//Make sure that o2 is sorted
		if ((j>1) && posO2[j] < posO2[j-1]){
			std::cout << "LCIS ALERT : o2 not sorted"<<std::endl;
			return 0;
		}
		last = posO2[j];
	}
	...
	for (int i = 0; i < (int)oh1.size(); i++){
		...
		std::sort(oh1[i].begin(), oh1[i].end());
		nbMarker = nbMarker + (int)oh1[i].size();
		if(VERBOSE){
			for (int j = 0; j < (int)oh1[i].size(); j++){
				std::cout << oh1[i][j];
				if (j != (int)oh1[i].size()-1) std::cout << " ";
