...
#' @param showName a boolean indicating whether or not marker names should be displayed (for large datasets displaying marker names often result in overly cluttered charts)
#' @param highlight a vector containg the names of the markers to be highlight for this graphic. The lines linking those markers will be drawn in dark grey, while others (e.g. those not part of the LCS) will be drawn in red instead of being masked as with the tokeep option.
...
#' res <- LCS(order_1,order_2)
#' show_connection(order_1, order_2, tokeep=res$LCS)
#' show_connection(order_1, order_2, highlight =res$LCS)

# A function that plot the relationship between 2 lists:
show_connection <- function(order_1, order_2, tokeep=NA, showName=TRUE, highlight=NA){
  
  # Prepare the data: merge the 2 lists and make a 'long' or 'tidy' format:
  don <- merge(order_1, order_2, by.x="V1", by.y="V1", all=T) %>%
  ...
  p <- p +
    geom_segment( x=1, xend=1, y=-max(order_1$V2), yend=0, color="black", size=0.3) +
    geom_segment( x=2, xend=2, y=-max(order_2$V2), yend=0, color="black", size=0.3) +
    geom_point() +
    geom_line() +
    theme_void() +
    theme(legend.position="none") +
    xlab("") +
    ylim( max(c(order_1$V2, order_2$V2)), 0)
